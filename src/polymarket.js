...
// Market tags (for category routing), cached 1h per event.
const TAG_TTL_MS = 60 * 60 * 1000;
const tagCache = new Map();
export async function fetchEventTags(eventSlug) {
  if (!eventSlug) return [];
  const cached = tagCache.get(eventSlug);
  if (cached && Date.now() - cached.at < TAG_TTL_MS) return cached.tags;
  let tags = [];
  try {
    const res = await fetch(`${GAMMA_API}/events?slug=${encodeURIComponent(eventSlug)}`, { headers });
    if (res.ok) {
      const data = await res.json();
      const ev = Array.isArray(data) ? data[0] : data?.events?.[0];
      tags = (ev?.tags || []).map((t) => String(t.label || t.slug || ""));
    }
  } catch (err) {
    console.warn(`[tags] ${eventSlug}: ${err.message}`);
  }
  tagCache.set(eventSlug, { at: Date.now(), tags });
  return tags;
}

// Profile-accurate PnL (last 7/14/30 days).
